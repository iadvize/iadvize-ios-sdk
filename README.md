...
Download the [IAdvizeConversationSDK.framework](https://github.com/iadvize/iadvize-ios-sdk/tree/master/IAdvizeConversationSDK.framework) from the Github repository and then copy the `IAdvizeConversationSDK.framework` in your project/workspace (a drag & drop in the `Embedded Binaries` of your app target is the easiest way).
